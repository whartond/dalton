...
def get_engine_conf_file(sensor):
    ...
    try:
        ...
        try:
            ...
            filelist = [
                f for f in os.listdir(epath) if os.path.isfile(os.path.join(epath, f))
            ]
...
def post_job_results(jobid):
    ...
    if redis.exists("%s-time" % jobid) and (
        ...
        return Response(
            "Error: job results already exist.",
            mimetype="text/plain",
            headers={"X-Dalton-Webapp": "Error"},
        )
...
def get_ajax_job_status_msg(jobid):
    ...
    if stat_code:
        ...
        if r_status_msg:
            ...
            return Response(
                "Unknown", mimetype="text/plain", headers={"X-Dalton-Webapp": "OK"}
            )
    else:
        return Response(
            "Invalid Job ID: %s" % jobid,
...
def sensor_get_job(id):
    ...
    if not validate_jobid(id):
        ...
        return render_template(
            "/dalton/error.html",
            jid=id,
            msg=[f"Bad jobid, invalid characters in: '{id}'"],
        )
    ...
    if os.path.exists(path):
        ...
        return render_template(
            ...
            jid=id,
            msg=[
                f"Job {id} does not exist on disk.  It is either invalid or has been deleted."
            ],
        )
...
def page_coverage_jid(jid, error=None):
    ...
    if not re.match(r"^[a-f0-9]{16}$", jid):
        return render_template(
            "/dalton/error.html", jid="", msg=["Not a valid job ID."]
        )

    jobzip_path = os.path.join(f"{JOB_STORAGE_PATH}", f"{jid}.zip")
    if not os.path.isfile(jobzip_path):
        return render_template(
            "/dalton/error.html", jid=jid, msg=[f"Job with ID {jid} does not exist."]
        )
        ...
        for sensor in redis.smembers("sensors"):
            try:
                ...
                return render_template(
                    "/dalton/error.html",
                    jid=None,
                    msg="Error getting sensor list for %s.  Error:\n%s" % (tech, e),
                )
        try:
            ...
            sensors.sort(
                key=lambda v: LooseVersion(
                    prefix_strip(v.split("/", 2)[1], prefixes=["rust_"])
                ),
                reverse=True,
            )
        except Exception:
...
def page_coverage_default(sensor_tech, error=None):
    ...
    if sensor_tech is None:
        return render_template(
            "/dalton/error.html", jid="", msg=["No Sensor technology selected for job."]
        )
    elif not re.match(r"^[a-zA-Z0-9\_\-\.]+$", sensor_tech):
        return render_template(
            "/dalton/error.html",
            jid="",
            msg=[f"Invalid Sensor technology requested: {sensor_tech}"],
        )
        ...
        return render_template(
            ...
            jid="",
            msg=[
                "Page expired.  Please resubmit your job or access it from the queue."
            ],
        )
    ...
    if not os.path.isdir(conf_dir) and not sensor_tech.startswith("zeek"):
        return render_template(
            ...
            jid="",
            msg=[
                f"No engine configuration directory for '{sensor_tech}' found ({conf_dir})."
            ],
        )
...
def page_show_job(jid):
    ...
    if not status:
        ...
        return render_template(
            ...
            jid=jid,
            msg=[
                "Invalid Job ID. Job may have expired.",
                "By default, jobs are only kept for %d seconds; teapot jobs are kept for %s seconds."
                % (REDIS_EXPIRE, TEAPOT_REDIS_EXPIRE),
            ],
        )
    elif int(status) != STAT_CODE_DONE:
        # job is queued or running
        return render_template(
            "/dalton/coverage-summary.html", page="", job_id=jid, tech=tech
        )
    else:
        ...
        if len(eve) > 0:
            ...
            try:
                ...
                event_types = set(
                    [item["event_type"] for item in eve_list if "event_type" in item]
                )
...
def extract_pcaps(archivename, pcap_files, job_id, dupcount):
    ...
    if archivename.lower().endswith(".zip"):
        ...
        try:
            ...
            msg = "Problem extracting ZIP file '%s': %s" % (
                os.path.basename(archivename),
                e,
            )
            logger.error(msg)
        ...
        ".gz",
        ".gzip",
    ] and os.path.splitext(os.path.splitext(archivename)[0])[1].lower() not in [".tar"]:
        # gzipped file
        try:
...
def page_coverage_summary():
    ...
    if request.form.get("fspcap"):
        ...
        pcap_files.append(
            {
                "filename": fspcap,
                "pcappath": os.path.join(FS_PCAP_PATH, os.path.basename(fspcap)),
            }
        )
    ...
    if job_zip is not None and re.match(r"^[a-f0-9]{16}\.zip$", job_zip):
        ...
        if not os.path.isfile(filepath):
            return render_template(
                "/dalton/error.html",
                jid="",
                msg=[f"Zip file for {filename} does not exist"],
            )
        ...
        try:
            ...
            for pcap_file in coverage_pcaps:
                if (
                    ...
                    if os.path.splitext(pcap_file.filename)[1].lower() in [
                        ".zip",
                        ".tar",
                        ".gz",
                        ".tgz",
                        ...
                        if err_msg:
                            delete_temp_files(job_id)
                            return render_template(
                                "/dalton/error.html", jid="", msg=[err_msg]
                            )
                    else:
    ...
    if not valid_sensor_tech:
        ...
        return render_template(
            ...
            jid="",
            msg=[
                f"There are no sensors that support sensor technology '{sensor_tech}'."
            ],
        )
    ...
    if len(form_pcap_files) == 0 and len(pcap_files) == 0:
        ...
        delete_temp_files(job_id)
        return render_template(
            "/dalton/error.html", jid="", msg=["You must specify at least one ruleset."]
        )
    else:
        ...
        if sensor_tech_engine is None or sensor_tech_version is None:
            ...
            return render_template(
                "/dalton/error.html",
                jid="",
                msg=[f"Bad sensor_tech string submitted: '{sensor_tech}'."],
            )
        ...
        if (
            len(pcap_files) > 1
            and sensor_tech.startswith("suri")
            and LooseVersion(sensor_tech_version) < LooseVersion("4.1")
            and not bSplitCap
        ):
            if not MERGECAP_BINARY:
                ...
                return render_template(
                    ...
                    jid=job_id,
                    msg=[
                        "No mergecap binary found on Dalton Controller.",
                        "Unable to process multiple pcaps for this Suricata job.",
                    ],
                )
            combined_file = "%s/combined-%s.pcap" % (
                os.path.join(TEMP_STORAGE_PATH, job_id),
                job_id,
            )
            ...
            try:
                ...
                mergecap_output = subprocess.Popen(
                    mergecap_command,
                    shell=True,
                    stderr=subprocess.STDOUT,
                    stdout=subprocess.PIPE,
                ...
                return render_template(
                    "/dalton/error.html",
                    jid="",
                    msg=["Could not merge pcaps.  Error:", f"{e}"],
                )
        ...
        if request.form.get("optionProdRuleset") and request.form.get(
            "optionEnableAllRules"
        ):
        ...
        try:
            ...
            if sensor_tech_engine == "suricata" and LooseVersion(
                sensor_tech_version
            ) < LooseVersion("2.1"):
                bGetBufferDumps = False
            if sensor_tech_engine == "snort" and LooseVersion(
                sensor_tech_version
        ...
        if request.form.get("optionCustomRuleset") and request.form.get(
            "custom_ruleset"
        ):
            bCustomRules = True
            ...
            for line in custom_rules.split("\n"):
                ...
                if (len(line) > 0) and not re.search(r"^[\x00-\x7F]+$", line):
                    ...
                    return render_template(
                        ...
                        jid="",
                        msg=[
                            "Invalid rule. Only ASCII characters are allowed in the literal representation of custom rules.",
                            "Please encode necessary non-ASCII characters appropriately.  Rule:",
                            f"{line}",
                        ],
                    )
                ...
                if sensor_tech.startswith("snort") or sensor_tech.startswith("suri"):
                    ...
                    if not re.search(
                        r"^(alert|log|pass|activate|dynamic|drop|reject|sdrop|event_filter|threshold|suppress|rate_filter|detection_filter)\s",
                        line,
                    ):
                        fh.close()
                        delete_temp_files(job_id)
                    ...
                    if (
                        not line.endswith(")")
                        and not line.startswith("event_filter")
                        and not line.startswith("threshold")
                        and not line.startswith("suppress")
                        and not line.startswith("rate_filter")
                        and not line.startswith("detection_filter")
                    ):
                        fh.close()
                        delete_temp_files(job_id)
                        return render_template(
                            ...
                            jid="",
                            msg=[
                                "Invalid rule; does not end with closing parenthesis.  Rule:",
                                f"{line}",
                            ],
                        )
                    ...
                    if (
                        ...
                        return render_template(
                            ...
                            jid="",
                            msg=[
                                "Invalid rule, last rule option must end with semicolon.  Rule:",
                                f"{line}",
                            ],
                        )
                    ...
                    if (
                        not re.search(
                            r"(^[^\x28]+\x28\s*|\s|\x3B)sid\s*\:\s*\d+\s*\x3B", line
                        )
                        and not line.startswith("event_filter")
                        and not line.startswith("threshold")
                        and not line.startswith("suppress")
                        ...
                        line = re.sub(
                            r"\x29$", " sid:%d;)" % (sid_base + sid_offset), line
                        )
                        sid_offset += 1
        ...
        if not sensor_tech:
            ...
            return render_template(
                ...
                jid="<not_defined>",
                msg=[
                    "Variable 'sensor_tech' not specified.  Please reload the submission page and try again."
                ],
            )
        ...
        if sensor_tech.startswith("suri"):
            ...
            conf_file = re.sub(
                r"(\w):\x20+(yes|no)([\x20\x0D\x0A\x23])",
                '\g<1>: "\g<2>"\g<3>',
                conf_file,
            )
            try:
                # read in yaml
                config = yaml.round_trip_load(
                    conf_file, version=(1, 1), preserve_quotes=True
                )
                ...
                if request.form.get("optionProdRuleset"):
                    ...
                    prod_ruleset_name = os.path.basename(
                        request.form.get("prod_ruleset")
                    )
                    if prod_ruleset_name.startswith(
                        JOB_STORAGE_PATH
                    ) and prod_ruleset_name.endswith(".zip"):
                        jobzip_path = os.path.join(
                            JOB_STORAGE_PATH, os.path.basename(prod_ruleset_name)
                        )
                        with zipfile.ZipFile(jobzip_path) as zf:
                            for f in zf.namelist():
                ...
                if (
                    "logging" in config
                    and "default-log-level" in config["logging"]
                    and config["logging"]["default-log-level"] == "notice"
                ):
                ...
                llist = [
                    list(config["logging"]["outputs"][i].keys())[0]
                    for i in range(0, len(config["logging"]["outputs"]))
                ]
                olist = [
                    list(config["outputs"][i].keys())[0]
                ...
                if "file" in llist:
                    try:
                        slog_level = config["logging"]["outputs"][llist.index("file")][
                            "file"
                        ]["level"]
                    except Exception as e:
                        logger.warning(
                ...
                if bGetOtherLogs:
                    ...
                    if "alert-debug" in olist:
                        config["outputs"][olist.index("alert-debug")] = (
                            alert_debug_config
                        )
                    else:
                ...
                try:
                    if bGetEveLog:
                        # Enable EVE Log
                        config["outputs"][olist.index("eve-log")]["eve-log"][
                            "enabled"
                        ] = True
                ...
                if bTrackPerformance:
                    ...
                    if "rules" not in config["profiling"]:
                        ...
                        config["profiling"]["rules"]["filename"] = (
                            "dalton-rule_perf.log"
                        )
                ...
                engine_conf_file = os.path.join(
                    TEMP_STORAGE_PATH, f"{job_id}_suricata.yaml"
                )
                engine_conf_fh = open(engine_conf_file, "w")
                engine_conf_fh.write(
                    yaml.round_trip_dump(config, version=(1, 1), explicit_start=True)
                )
                ...
                return render_template(
                    "/dalton/error.html",
                    jid="",
                    msg=["Error processing YAML file(s):", f"{e}"],
                )
        else:
            ...
            if sensor_tech.startswith("snort"):
                ...
                conf_file = new_conf
                engine_conf_file = os.path.join(
                    TEMP_STORAGE_PATH, f"{job_id}_snort.conf"
                )
        ...
        for splitcap in pcap_files:
            ...
            try:
                if bSplitCap:
                    zf.write(
                        splitcap["pcappath"],
                        arcname=os.path.basename(splitcap["filename"]),
                    )
                else:
                    for pcap in pcap_files:
                        zf.write(
                            pcap["pcappath"], arcname=os.path.basename(pcap["filename"])
                        )
                if request.form.get("optionProdRuleset"):
                    ...
                    if not ruleset_path:
                        ...
                        return render_template(
                            "/dalton/error.html",
                            jid=jid,
                            msg=["No defined ruleset provided."],
                        )
                    if ruleset_path.startswith(
                        JOB_STORAGE_PATH
                    ) and ruleset_path.endswith(".zip"):
                        jobzip_path = os.path.join(
                            JOB_STORAGE_PATH, os.path.basename(ruleset_path)
                        )
                        if not os.path.exists(jobzip_path):
                            ...
                            return render_template(
                                ...
                                jid=jid,
                                msg=[
                                    "Ruleset does not exist on Dalton Controller: %s; ruleset-path: %s"
                                    % (prod_ruleset_name, ruleset_path)
                                ],
                            )
                            ...
                            for f in jobzf.namelist():
                                if f.endswith(".rules") and f != "dalton-custom.rules":
                                    ruleset_path = os.path.join(
                                        TEMP_STORAGE_PATH, job_id, f
                                    )
                                    open(ruleset_path, "w").write(
                                        jobzf.read(f).decode()
                                    )
                    ...
                    if (
                        (
                            not ruleset_path.startswith(RULESET_STORAGE_PATH)
                            and not ruleset_path.startswith(TEMP_STORAGE_PATH)
                        )
                        or ".." in ruleset_path
                        or not re.search(
                            r"^[a-z0-9\/\_\-\.]+$", ruleset_path, re.IGNORECASE
                        )
                    ):
                        delete_temp_files(job_id)
                        return render_template(
                            ...
                            jid=jid,
                            msg=[
                                "Invalid ruleset submitted: '%s'." % prod_ruleset_name,
                                "Path/name invalid.",
                            ],
                        )
                        ...
                        if bEnableAllRules or bShowFlowbitAlerts:
                            modified_rules_path = "%s/%s_prod_modified.rules" % (
                                TEMP_STORAGE_PATH,
                                job_id,
                            )
                            regex = re.compile(
                                r"^#+\s*(alert|log|pass|activate|dynamic|drop|reject|sdrop)\s"
                            )
                            ...
                            for line in prod_rules_fh:
                                ...
                                if bShowFlowbitAlerts:
                                    line = re.sub(
                                        r"([\x3B\s])flowbits\s*\x3A\s*noalert\s*\x3B",
                                        "\g<1>",
                                        line,
                                    )
                ...
                if engine_conf_file:
                    zf.write(
                        engine_conf_file, arcname=os.path.basename(engine_conf_file)
                    )
                ...
                try:
                    ...
                    if json_response["data"] is None:
                        ...
                        json_response["error_msg"] = (
                            f"No data found for '{requested_data}' for Job ID {jid}"
                        )
...
@dalton_blueprint.route(
    "/dalton/controller_api/v2/<jid>/<requested_data>", defaults={"raw": ""}
)
@dalton_blueprint.route(
...
def controller_api_get_current_sensors(engine):
    ...
    if engine is None or engine == "" or engine not in supported_engines:
        return Response(
            "Invalid 'engine' supplied.  Must be one of %s.\nExample URI:\n\n/dalton/controller_api/get-current-sensors/suricata"
            % supported_engines,
            status=400,
            mimetype="text/plain",
            headers={"X-Dalton-Webapp": "OK"},
    ...
    try:
        sensors.sort(
            key=lambda v: LooseVersion(
                prefix_strip(v.split("/", 1)[1], prefixes="rust_")
            ),
            reverse=True,
        )
    except Exception:
