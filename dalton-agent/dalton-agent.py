...
file_handler = RotatingFileHandler(
    "/var/log/dalton-agent.log", "a", 1 * 1024 * 1024, 10
)
...
def get_engine_version(path):
    ...
    try:
        ...
        stdout, stderr = process.communicate()
        regex = re.compile(
            r"(Version|(Suricata|zeek) version)\s+(?P<version>\d+[\d\x2E\x2D\5FA-Za-z]*)"
        )
        if stderr:
        ...
        if "suricata" in engine and version.split(".")[0] == "4":
            process = subprocess.Popen(
                '%s --build-info | grep "Rust support"' % path,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                shell=True,
            )
...
if USE_SURICATA_SOCKET_CONTROL:
    if os.path.isdir(SURICATA_SC_PYTHON_MODULE):
        ...
        os.path.abspath(
            os.path.join(
                SURICATA_SC_PYTHON_MODULE, "..", "scripts", "suricatasc", "src"
            )
        )
    ):
...
if "no_proxy" not in os.environ:
    ...
    os.environ["no_proxy"] = "%s,%s" % (
        os.environ["no_proxy"].rstrip(","),
        dalton_web_container,
    )
...
class SocketController:
    ...
    def send_command(self, command):
        ...
        if cmdret["return"] == "NOK":
            print_error(
                '"NOK" response received from socket command; message: %s'
                % json.dumps(cmdret["message"])
            )
    ...
    def stop_suricata_daemon(self):
        ...
        try:
            ...
            print_error(
                f"Problem shutting down old Suricata instance in stop_suricata_daemon(): {e}"
            )
        finally:
            self.suricata_is_running = False
    ...
    def start_suricata_daemon(self, config):
        ...
        print_debug(
            f"Starting suricata thread with the following command:\n{suricata_command}"
        )
        ...
        with open(suricata_logging_outputs_file, "r") as suri_output_fh:
            logger.debug(
                "tailing '%s' to see when engine has started up fully"
                % suricata_logging_outputs_file
            )
...
def send_update(msg, job_id=None):
    ...
    req = urllib.request.Request(
        url, urllib.parse.urlencode(params).encode("utf-8"), HTTP_HEADERS
    )
    try:
...
def request_job():
    try:
        data = (
            urllib.request.urlopen(req_job_url, timeout=URLLIB_TIMEOUT)
            .read()
            .decode("utf-8")
        )
    except Exception:
        raise Exception(
            f"Error in sensor '{SENSOR_UID}'. "
    ...
    if data == "sleep":
        ...
        try:
            ...
            print_error(
                f"Problem loading json from Dalton Controller; could not parse job id from data: '{data}'."
            )
        return job
...
def post_results(json_data):
    ...
    url = "%s/results/%s?SENSOR_UID=%s&apikey=%s" % (
        DALTON_API,
        JOB_ID,
        SENSOR_UID,
        API_KEY,
    ...
    req = urllib.request.Request(
        url, urllib.parse.urlencode(json_data).encode("utf-8"), HTTP_HEADERS
    )
    try:
        urllib.request.urlopen(req, timeout=URLLIB_TIMEOUT)
    except Exception as e:
...
def error_post_results(error_msg):
    ...
    results_dict = {}
    results_dict["error"] = (
        "Unexpected error on Dalton Agent '%s', please try your job again or contact admin with this message (see 'About' page for contact info).  Error message:\n\n%s"
        % (SENSOR_UID, error_msg)
    )
    results_dict["status"] = "ERROR"
    results_dict["ids"] = ""
    results_dict["alert"] = "*** No Alerts ***\n"
...
def process_snort_alerts():
    ...
    for alert_file in glob.glob(
        os.path.join(IDS_LOG_DIRECTORY, "alert-full_dalton-agent*")
    ):
            ...
            if os.path.exists(TCPDUMP_BINARY):
                for pcap in PCAP_FILES:
                    ...
                    if (
                        len(
                            subprocess.Popen(
                                "%s -nn -q -c 1 -r %s -p tcp 2>/dev/null"
                                % (TCPDUMP_BINARY, pcap),
                                shell=True,
                                stdout=subprocess.PIPE,
                            ).stdout.read()
                        ...
                        if (
                            len(
                                subprocess.Popen(
                                    '%s -nn -q -c 1 -r %s "tcp[tcpflags] & tcp-syn != 0" 2>/dev/null'
                                    % (TCPDUMP_BINARY, pcap),
                                    shell=True,
                                    stdout=subprocess.PIPE,
                                ).stdout.read()
                            ...
                            if (
                                len(
                                    subprocess.Popen(
                                        '%s -nn -q -c 1 -r %s "ip6 and tcp and ip6[0x35] & 0x2 != 0" 2>/dev/null'
                                        % (TCPDUMP_BINARY, pcap),
                                        shell=True,
                                        stdout=subprocess.PIPE,
                                    ).stdout.read()
                ...
                print_debug(
                    "In check_pcaps() -- no tcpdump binary found at %s" % TCPDUMP_BINARY
                )
        except Exception as e:
    ...
    try:
        for pcap in PCAP_FILES:
            ...
            if magic.lower() == "a1b2c3d4":
                ...
                print_debug(
                    "in check_pcaps() - Pcap Byte-Order Magic field not found in file '%s'.  Is this a valid pcap?"
                    % os.path.basename(pcap)
                )
                continue
            ...
            if little_endian:
                snaplen = struct.unpack(
                    "<i", bytes[snaplen_offset : snaplen_offset + 4]
                )[0]
            else:
                snaplen = struct.unpack(
                    ">i", bytes[snaplen_offset : snaplen_offset + 4]
...
def run_snort():
    ...
    snort_command = (
        "%s -Q --daq dump --daq-dir /usr/lib/daq/ --daq-var load-mode=read-file --daq-var file=/tmp/inline-out.pcap -l %s -c %s -k none -X --conf-error-out --process-all-events --treat-drop-as-alert --pcap-dir=%s --buffer-dump-alert=%s 2>&1"
        % (
            IDS_BINARY,
            IDS_LOG_DIRECTORY,
            IDS_CONFIG_FILE,
            os.path.split(PCAP_FILES[0])[0],
            IDS_BUFFERS_LOG,
        )
    )
    ...
    subprocess.call(
        snort_command, shell=True, stderr=subprocess.STDOUT, stdout=snort_output_fh
    )
...
def run_suricata_sc():
    ...
    logger.debug(
        "OLD config_hash: %s, ruleset_hash: %s"
        % (SCONTROL.config_hash, SCONTROL.ruleset_hash)
    )
    if (
        not (
            ruleset_hash == SCONTROL.ruleset_hash
            and config_hash == SCONTROL.config_hash
        )
    ) and SCONTROL.suricata_is_running:
        # if hashes don't match, shutdown suri via socket, start new suri, update hashes, run
        print_debug(
            "Suricata Socket Control: new hashes found, restarting Suricata....."
        )
    ...
    if SC_FIRST_RUN:
        ...
        if SENSOR_ENGINE_VERSION_ORIG in bug4225_versions:
            ...
            logger.debug(
                "Sending dummy pcap to socket control to work around Redmine 4225"
            )
            resp = SCONTROL.send_command(f"pcap-file {dummy_pcap_file} /tmp")
            logger.debug(f"Sent dummy pcap. Response: {resp}")
            while (
                int(SCONTROL.send_command("pcap-file-number")) > 0
                or SCONTROL.send_command("pcap-current") != '"None"'
            ):
    ...
    logger.debug(
        "In run_suricata_sc(): all pcaps done running ... closing connection to socket."
    )
    SCONTROL.close()
...
def run_suricata():
    ...
    subprocess.call(
        suricata_command, shell=True, stderr=subprocess.STDOUT, stdout=suri_output_fh
    )
...
def generate_fast_pattern():
    ...
    if SENSOR_ENGINE.startswith("suri"):
        if not IDS_BINARY:
            print_error("No Suricata binary found on system.")
        suricata_command = "%s -c %s -l %s --engine-analysis" % (
            IDS_BINARY,
            IDS_CONFIG_FILE,
            IDS_LOG_DIRECTORY,
        )
        print_debug(
            "Running suricata with the following command to get fast pattern info:\n%s"
            % suricata_command
        )
        ...
        for log_name in other_logs:
            if not os.path.exists("%s/%s" % (IDS_LOG_DIRECTORY, other_logs[log_name])):
                ...
                if log_name_new != other_logs[log_name]:
                    print_debug(
                        "Log file '%s' not present, trying '%s'..."
            ...
            if os.path.exists("%s/%s" % (IDS_LOG_DIRECTORY, other_logs[log_name])):
                ...
                print_debug(
                    "Requested log file '%s' not present, skipping."
                    % other_logs[log_name]
                )
...
def check_for_errors(tech):
    ...
    try:
        ...
        for line in ids_log_fh:
            if tech.startswith("suri"):
                if (
                    ...
                    or line.startswith("Failed to parse configuration file")
                ):
                    error_lines.append(line)
                    if "bad dump file format" in line or "unknown file format" in line:
                        error_lines.append(
                            "Bad pcap file(s) submitted to Suricata. Pcap files should be in libpcap format (pcapng is not supported in older Suricata versions).\n"
                        )
        ...
        print_error(
            "Error reading IDS output file '%s'. Error:\n\n%s" % (JOB_IDS_LOG, e)
        )
...
def process_unified2_logs():
    ...
    u2_combined_file = os.path.join(
        IDS_LOG_DIRECTORY, "dalton-unified2-combined.alerts"
    )
    ...
    try:
        ...
        print_debug(
            "Error processing unified2 files and base64 encoding them for transmission ... bailing. Error: %s"
            % e
        )
        return
...
def process_performance_logs():
    ...
    if len(glob.glob(os.path.join(IDS_LOG_DIRECTORY, "dalton-rule_perf*"))) > 0:
        for perf_file in glob.glob(
            os.path.join(IDS_LOG_DIRECTORY, "dalton-rule_perf*")
        ):
        ...
        print_debug(
            "No rules performance log(s) found. File '%s' does not exist."
            % "dalton-rule_perf*"
        )
...
def run_zeek(json_logs):
    ...
    if (
        len(
            [
                f
                for f in os.listdir("/opt/dalton-agent/zeek_scripts/")
                if not f.startswith(".")
            ]
        )
        > 0
    ...
    subprocess.call(
        zeek_command, shell=True, stderr=zeek_error_fh, stdout=zeek_output_fh
    )
...
def submit_job(job_id, job_directory):
        ...
        try:
            ...
            if useSuricataSC != USE_SURICATA_SOCKET_CONTROL:
                if (
                    ...
                    and float(
                        ".".join(
                            prefix_strip(SENSOR_ENGINE_VERSION_ORIG).split(".")[:2]
                        )
                    )
                    < 3.0
                ):
    ...
    try:
        manifest_data[0]["get-engine-stats"]
    except Exception:
        pass
    ...
    if os.path.isdir(cdir):
        ...
        for file in file_list:
            if os.path.isfile(os.path.join(cdir, file)):
                shutil.copyfile(
                    os.path.join(cdir, file), os.path.join(JOB_DIRECTORY, file)
                )
    ...
    if not SENSOR_ENGINE.startswith("zeek"):
        try:
            IDS_CONFIG_FILE = os.path.join(
                JOB_DIRECTORY, os.path.basename(manifest_data[0]["engine-conf"])
            )
        except Exception:
    ...
    try:
        PCAP_FILES = [
            os.path.join(JOB_DIRECTORY, PCAP_DIR, os.path.basename(cap))
            for cap in manifest_data[0]["pcaps"]
        ]
        for pcap_file in PCAP_FILES:
            ...
            shutil.move(
                os.path.join(
                    os.path.dirname(os.path.dirname(pcap_file)),
        ...
        logger.debug(
            "Problem moving pcaps to directory '%s': %s"
            % (os.path.join(JOB_DIRECTORY, PCAP_DIR), e)
        )
...
while True:
    ...
    try:
        ...
        if job is not None:
            ...
            logger.debug(
                "Downloaded zip for %s successfully. Extracting file %s"
                % (JOB_ID, zf_path)
            )
            send_update(
                "Downloaded zip for %s successfully; extracting..." % JOB_ID, JOB_ID
            )
            # JOB_DEBUG_LOG not defined yet so can't call print_debug() here
            # print_debug("Extracting zip file for job id %s" % JOB_ID)
            JOB_DIRECTORY = "%s/%s_%s" % (
                STORAGE_PATH,
                JOB_ID,
            ...
            try:
                ...
                if JOB_ERROR_LOG:
                    msg = (
                        "Dalton Agent error in sensor '%s' while processing job %s. Exception in submit_job().  Please re-submit or contact admin with this message (see 'About' page for contact info).  Error message:\n\n%s"
                        % (SENSOR_UID, JOB_ID, e)
                    )
                    fh = open(JOB_ERROR_LOG, "a")
                    fh.write("%s\n" % msg)
                    ...
                    error_post_results(
                        "Dalton Agent Error in sensor '%s' while processing job %s. Error:\n%s"
                        % (SENSOR_UID, JOB_ID, e)
                    )
            ...
            print_debug(
                "Total Processing Time (includes job download time): %d seconds"
                % TOTAL_PROCESSING_TIME
            )
        ...
        logger.debug(
            "General Dalton Agent exception caught. Error:\n%s\n%s"
        ...
        if JOB_ID:
            ...
            try:
                ...
                logger.info(
                    "Successfully sent error message to controller for jobid %s"
                    % JOB_ID
                )
            except Exception as e:
                logger.error(
                    "Could not communicate with controller to send error info for jobid %s; is the Dalton Controller accepting network communications? Error:\n%s"
                    % (JOB_ID, e)
                )
                time.sleep(ERROR_SLEEP_TIME)
        else:
            logger.error(
                "Agent Error -- Is the Dalton Controller accepting network communications?"
            )
            sys.stdout.flush()
            time.sleep(ERROR_SLEEP_TIME)
#    finally:
